...
(function() {
  ...
  function loadProSettingsToInputs() {
    ...
    if (window.G.bgTheme === undefined) window.G.bgTheme = "spring";
    ...
    if (bgThemeSelect) bgThemeSelect.value = g.bgTheme || "spring";
  ...
  function applyPreviewTheme() {
    ...
    if (isPro && g.useBgImage) {
      ...
      if (g.bgTheme === 'custom' && g.bgCustomUrl) {
      ...
      bgFileInput.addEventListener("change", (e) => {
        if (e.target.files.length > 0) {
          handleImageFile(e.target.files[0], "backgrounds", (dataUrl) => {
            ...
            const themeSelect = document.getElementById("editor-pro-bg-theme-select");
            if (themeSelect) themeSelect.value = ""; // カスタム選択中
  ...
  window.addEventListener("load", async () => {
    try {
      ...
      // プレビュー表示・非表示・縮小ボタンのアクション
      const pane = document.querySelector(".editor-live-preview-pane");
      const btnScaleDown = document.getElementById("btn-preview-scale-down");
      const btnScaleReset = document.getElementById("btn-preview-scale-reset");
      const btnClose = document.getElementById("btn-preview-close");
      const btnRestore = document.getElementById("btn-preview-restore");

      if (btnScaleDown && btnScaleReset && pane) {
        btnScaleDown.addEventListener("click", () => {
          pane.classList.add("preview-scaled");
          btnScaleDown.style.display = "none";
          btnScaleReset.style.display = "inline-block";
        });
        btnScaleReset.addEventListener("click", () => {
          pane.classList.remove("preview-scaled");
          btnScaleDown.style.display = "inline-block";
          btnScaleReset.style.display = "none";
        });
      }

      if (btnClose && btnRestore && pane) {
        ...
        btnRestore.addEventListener("click", () => {
          ...
          if (window.innerWidth <= 1180) {
            pane.classList.add("preview-scaled");
            if (btnScaleDown) btnScaleDown.style.display = "none";
            if (btnScaleReset) btnScaleReset.style.display = "inline-block";
          }
        });
      }

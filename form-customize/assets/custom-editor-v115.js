      ...
      sections: [
        ...
        },
        {
          ...
          questions: [
            ...
            { id: "q_hc_street", type: "text", title: "町名・番地", description: "数字・ハイフンは全角統一されます。末尾に必ず【番地・号】を追記してください。", required: true, dataKey: "street", options: [] },
            { id: "q_hc_building", type: "text", title: "建物名・部屋番号", description: "ビル名・階数・部屋番号等がある場合はご入力ください（数字・ハイフンは全角統一）", required: false, dataKey: "building", options: [] },
        ...
        },
        {
          ...
          questions: [
            ...
            { id: "q_hi_street", type: "text", title: "町名・番地", description: "数字・ハイフンは全角統一されます。末尾に必ず【番地・号】を追記してください。", required: true, dataKey: "street", options: [] },
            { id: "q_hi_building", type: "text", title: "建物名・部屋番号", description: "マンション名・アパート名・部屋番号等がある場合はご入力ください（数字・ハイフンは全角統一）", required: false, dataKey: "building", options: [] },
...
// Bank listeners moved to dedicated setup functions below
  ...
  if (originalDt) {
    ...
    function executeApplyPreset(val, activeSec, baseTime) {
      ...
      try {
        ...
        console.warn('[executeApplyPreset] window.le error suppressed:', e);
      }
      try {
        if (typeof window.Q === 'function') window.Q();
      } catch(e) {
        console.warn('[executeApplyPreset] window.Q error suppressed:', e);
      }
      try {
        if (window.S) window.S(true);
      } catch(e) {
        console.warn('[executeApplyPreset] window.S error suppressed:', e);
      }
      try {
        if (window.x) window.x();
      } catch(e) {
        console.warn('[executeApplyPreset] window.x error suppressed:', e);
      }
      try {
        if (typeof window.syncFormsToCloud === 'function') {
          window.syncFormsToCloud(null, true);
        } else if (typeof syncFormsToCloud === 'function') {
          syncFormsToCloud(null, true);
        }
      } catch(e) {
        console.error('[executeApplyPreset] syncFormsToCloud error:', e);
      }
      try {
        renderLivePreview();
      } catch(e) {
    ...
    activeSec.questions.forEach(q => {
      ...
      if (q.groupId) {
        if (!currentPreviewGrpEl || currentPreviewGrpId !== q.groupId) {
          ...
          // グループ一括同上チェックボックスプレビュー（デフォルトOFF: 未チェックで全入力欄を開く）
          if (q.sameAsAbove && q.sameAsAbove.enabled && (q.sameAsAbove.sourceType === 'group' || (q.sameAsAbove.sourceQuestionId && q.sameAsAbove.sourceQuestionId.startsWith('group:')))) {
            ...
            grpSameWrap.style.cssText = 'background: rgba(248, 250, 252, 0.9); border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; margin-bottom: 8px; display: flex; align-items: center; justify-content: space-between; gap: 10px;';
            const rawLabel = q.sameAsAbove.label || '前述のグループと同じ';
            grpSameWrap.innerHTML = `
              <label style="display:inline-flex; align-items:center; gap:8px; font-size:0.85rem; font-weight:700; color:#334155; margin:0; cursor:default;">
                <input type="checkbox" disabled style="margin: 0; accent-color:#059669;" />
  ...
  async function openShareUrlModal(formIndex) {
    ...
    // 未統合の変更がある場合はテスト送信タブ、統合済みの場合は本番タブを初期選択
    const pubStatus = checkFormPublishStatus(formObj);
    const defaultEnv = (pubStatus && !pubStatus.isSynced) ? 'test' : 'production';
    applyEnvTab(defaultEnv);
  ...
  function initShareUrlFeature() {
    ...
    if (shareBtn && !shareBtn._hooked) {
      ...
      shareBtn.addEventListener('click', (e) => {
        ...
        copyFormShareUrl(); // 未統合の変更がある場合はテスト送信リンク、統合済みの場合は本番用リンクを即時コピー
...
(function enforceAdminSessionAndHideLogout() {
  ...
  function countFormsQuestions(formList) {
    if (!Array.isArray(formList)) return 0;
    let total = 0;
    formList.forEach(f => {
    ...
    if (!forms) {
      if (window.U && Array.isArray(window.U) && window.U.length > 0) {
        if (window.G && window.W !== undefined && window.U[window.W]) {
          window.U[window.W] = window.G;
        }
        forms = JSON.parse(JSON.stringify(window.U));
      } else if (window.G) {
        forms = [JSON.parse(JSON.stringify(window.G))];
      } else {
        try {
          const raw = localStorage.getItem('form_customize_all_forms');
  ...
  async function loadFormsFromCloud() {
    ...
    try {
      let cloudForms = null;
      try {
        const res = await fetch('/api/forms?all=1&_t=' + Date.now());
        if (res.ok) {
          const data = await res.json();
      ...
      const cloudQCount = countFormsQuestions(cloudForms);

      // 🛡️ 逆流ロールバック防止ガード:
      // もしローカル側の質問数の方がクラウド側よりも多い場合、古いクラウドデータでローカルの最新ドラフトを破壊しない！
      // 逆にローカルの最新ドラフトを即座にクラウドへ書き戻す。
      if (localQCount > cloudQCount && cloudForms !== null) {
        console.warn(`[Cloud Sync Guard] Local draft has more questions (${localQCount}) than Cloud (${cloudQCount}). Preserving local and pushing to cloud!`);
        syncFormsToCloud(localForms, true);
        return;
      }
      ...
      if (cloudForms !== null && Array.isArray(cloudForms)) {
        ...
        if (cleanLocal.length > 0) {
          syncFormsToCloud(cleanLocal, true);
        }
      }
    } catch(err) {
  ...
  setTimeout(mountCloudSyncHeaderControls, 300);
  setTimeout(mountCloudSyncHeaderControls, 1000);
...
(function() {
  ...
  function downloadFormBulkInputTemplate(targetFormDef = null) {
    ...
    // 設問タイトルの出現頻度を集計（同名タイトルの検出）
    const titleCounts = {};
    questionCols.forEach(c => {
      const t = c.title;
      titleCounts[t] = (titleCounts[t] || 0) + 1;
    });

    // ヘッダー表示名の決定
    // グループ名がある場合は [グループ名] 設問名 (物理キー)
    // グループ名がない場合でも重複時は [セクション名] 設問名 (物理キー)
    // 物理キーを末尾に明記することで、人間が見ても識別でき、
    // インポート時にも Supabase物理カラムと 100% 確実に完全一致照合できる
    questionCols.forEach(c => {
      const isDup = titleCounts[c.title] > 1;
      let hName = c.title;
      if (c.groupTitle) {
        hName = `[${c.groupTitle}] ${c.title}`;
      } else if (isDup && c.sectionTitle) {
        hName = `[${c.sectionTitle}] ${c.title}`;
      }
      c.headerName = `${hName} (${c.dataKey})`;
    });
    ...
    // 1行目: グループ名＋設問タイトル＋物理キー（インポート時に物理キー完全一致で100%照合）
    const headerRow = questionCols.map(c => escapeCsv(c.headerName)).join(',');

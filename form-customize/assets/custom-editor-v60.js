...
(function() {
  ...
  function syncActiveIndexFromWindowG() {
    ...
    // 1. 通常フォームから検索
    let foundIndex = allForms.findIndex(f => (currentId && f.id === currentId) || (!currentId && f.title === currentTitle));
    if (foundIndex !== -1) {
    ...
    // 2. テンプレートマスタから検索
    foundIndex = templates.findIndex(t => (currentId && t.id === currentId) || (!currentId && t.title === currentTitle));
    if (foundIndex !== -1) {
      const savedIndex = localStorage.getItem('form_customize_active_index');
      const savedTemplateMode = localStorage.getItem('form_customize_is_template_mode');

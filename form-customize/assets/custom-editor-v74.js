...
(function() {
  ...
  (function setupLocalStorageHook() {
    const originalGetItem = localStorage.getItem;
    const originalSetItem = localStorage.setItem;

    localStorage.getItem = function(key) {
      if (key === 'form_customize_all_forms') {
        const isTemplateMode = originalGetItem.call(localStorage, 'form_customize_is_template_mode') === 'true';
        const activeTab = originalGetItem.call(localStorage, 'form_customize_active_tab') || 'dashboard';
        const isReactEditorActive = activeTab === 'editor' || activeTab === 'flowmap' || activeTab === 'preview';
        
        // 呼び出し元が React エディタコアであること（外部の createFormFromTemplate 等からの直呼び出しはフックしない）
        const stack = new Error().stack || '';
        const isFromReactCore = stack.includes('index-') || stack.includes('bundle');

        if (isTemplateMode && isReactEditorActive && isFromReactCore) {
          console.log('[LocalStorage Hook] Redirecting form_customize_all_forms -> form_customize_templates (read)');
          return originalGetItem.call(localStorage, 'form_customize_templates') || '[]';
        }
      }
    ...
    localStorage.setItem = function(key, value) {
      if (key === 'form_customize_all_forms') {
        ...
        // 呼び出し元が React エディタコアであること
        const stack = new Error().stack || '';
        const isFromReactCore = stack.includes('index-') || stack.includes('bundle');

        if (isTemplateMode && isReactEditorActive && isFromReactCore) {
          // 【セーフガード】書き込むデータが通常のフォーム一覧である場合は、テンプレート一覧へのリダイレクトをブロックする
          try {
            const parsed = JSON.parse(value);
            if (Array.isArray(parsed)) {
              const hasNormalUserForm = parsed.some(f => 
                f && (f.ownerId === 'user_own_editor' || f.isTemplateMode === false || /\d{4}\/\d{1,2}\/\d{1,2}/.test(f.title))
              );
              if (hasNormalUserForm) {
                console.log('[LocalStorage Hook] Blocked redirect to templates because value contains normal user forms.');
                return originalSetItem.call(localStorage, 'form_customize_all_forms', value);
              }
            }
          } catch(e) {
            console.error('[LocalStorage Hook] Failed to parse value for validation:', e);
          }

          console.log('[LocalStorage Hook] Redirecting form_customize_all_forms -> form_customize_templates (write)');
          return originalSetItem.call(localStorage, 'form_customize_templates', value);
        }
      }
  ...
  function migrateMisplacedForms() {
    try {
      const originalGetItem = localStorage.getItem;
      const originalSetItem = localStorage.setItem;
      ...
      let templates = [];
      try {
        templates = JSON.parse(originalGetItem.call(localStorage, 'form_customize_templates') || '[]');
      } catch(e) {}
      
      // もしテンプレートが完全に空か破損して消えてしまっている場合は、デフォルトテンプレートで初期化する
      if (!templates || templates.length === 0) {
        templates = JSON.parse(JSON.stringify(DEFAULT_TEMPLATES));
        console.log('[Migration] Templates were empty or corrupted. Re-initializing with defaults.');
        originalSetItem.call(localStorage, 'form_customize_templates', JSON.stringify(templates));
      }
      ...
      templates.forEach(t => {
        if (!t) return;
        // タイトルに「2026/」などの日付、あるいは年月日が含まれているものを移行対象とする
        const hasDate = /\d{4}\/\d{1,2}\/\d{1,2}/.test(t.title);
        if (hasDate) {
          console.log('[Migration] Migrating misplaced template back to forms:', t.title);
          t.isTemplateMode = false;
          t.ownerId = 'user_own_editor';
          allForms.push(t);
          migratedCount++;
        } else {
      ...
      if (migratedCount > 0) {
        originalSetItem.call(localStorage, 'form_customize_templates', JSON.stringify(updatedTemplates));
        originalSetItem.call(localStorage, 'form_customize_all_forms', JSON.stringify(allForms));
        console.log(`[Migration] Misplaced forms migrated successfully: ${migratedCount} items.`);
      }
    } catch(e) {
      console.error('[Migration] Failed to migrate misplaced forms:', e);
    }
  }
  ...
  function createFormFromTemplate(template) {
    ...
    // コピー元の最終更新時刻ではなく、作成された現在時刻を設定する
    const now = new Date();
    newForm.lastModified = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;

    allForms.push(newForm);

    // 【超重要】保存する前にテンプレートモードのフラグを解除しておくことで、フックの誤爆を防ぐ

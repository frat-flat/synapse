  ...
  function updateHeaderActiveFormTitle(title) {
    ...
    if (badge) {
      ...
      if (title && activeTab !== 'dashboard') {
        const isTemplateMode = localStorage.getItem('form_customize_is_template_mode') === 'true';
        const label = isTemplateMode ? "📝 編集中のテンプレート: " : "📝 編集中のフォーム: ";
        badge.innerHTML = `<span style="color: #718096; font-weight: 500; font-size: 0.72rem; margin-right: 4px;">${label}</span>${title}`;

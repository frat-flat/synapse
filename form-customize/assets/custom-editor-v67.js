  ...
  function syncActiveTabAndDropdown() {
    ...
    if (badge) {
      if (tabName !== 'dashboard') {
        const titleSpan = document.getElementById('header-active-form-title');
        ...
        if (isTemplateMode) {
          ...
          if (templates[activeIndex]) expectedTitle = templates[activeIndex].title;
        } else {
          let allForms = [];
          try { allForms = JSON.parse(localStorage.getItem('form_customize_all_forms') || '[]'); } catch(e) {}
          if (allForms[activeIndex]) expectedTitle = allForms[activeIndex].title;
        }
        
        // タイトル表示が存在しない、またはテキストが最新と乖離している場合は再生成

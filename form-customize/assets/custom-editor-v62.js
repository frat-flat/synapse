...
(function() {
          ...
          window.Z = function(tabName) {
            if (isFirstZCall) {
              ...
              if (tabName === 'dashboard' && savedTab && savedTab !== 'dashboard') {
                ...
                return originalZ(savedTab);
              }
            }
            return originalZ(tabName);
          };
        }
  ...
  function syncActiveIndexFromWindowG() {
    ...
    let allForms = [];
    try { allForms = JSON.parse(localStorage.getItem('form_customize_all_forms') || '[]'); } catch(e) {}
    let templates = [];
    try { templates = JSON.parse(localStorage.getItem('form_customize_templates') || '[]'); } catch(e) {}
    
    const currentId = window.G.id;
    const currentTitle = window.G.title;
    
    // 1. 通常フォームから検索
    let foundIndex = allForms.findIndex(f => {
      if (currentId && f.id && f.id === currentId) return true;
      if (currentTitle && f.title === currentTitle) return true;
      return false;
    });
    if (foundIndex !== -1) {
      const savedIndex = localStorage.getItem('form_customize_active_index');
      const savedTemplateMode = localStorage.getItem('form_customize_is_template_mode');
      
      if (savedIndex !== foundIndex.toString() || savedTemplateMode !== 'false') {
        console.log(`[Extension] Syncing active index from window.G to Form #${foundIndex}`);
        localStorage.setItem('form_customize_active_index', foundIndex.toString());
    ...
    // 2. テンプレートマスタから検索
    foundIndex = templates.findIndex(t => {
      if (currentId && t.id && t.id === currentId) return true;
      if (currentTitle && t.title === currentTitle) return true;
      return false;
    });
    if (foundIndex !== -1) {
      const savedIndex = localStorage.getItem('form_customize_active_index');
      ...
      if (savedIndex !== foundIndex.toString() || savedTemplateMode !== 'true') {
        console.log(`[Extension] Syncing active index from window.G to Template #${foundIndex}`);
        localStorage.setItem('form_customize_active_index', foundIndex.toString());
        localStorage.setItem('form_customize_is_template_mode', 'true');
      }
      return;
    }
  }
  ...
  function syncActiveTabAndDropdown() {
    ...
    // アクティブなフォームのインデックス状態を window.G から常に逆引き同期
    syncActiveIndexFromWindowG();
    ...
    if (badge) {
      if (tabName !== 'dashboard') {
        const select = document.getElementById('header-active-form-select');
        const isTemplateMode = localStorage.getItem('form_customize_is_template_mode') === 'true';
        const expectedValue = isTemplateMode ? `template_${localStorage.getItem('form_customize_active_index')}` : `form_${localStorage.getItem('form_customize_active_index')}`;
        
        // プルダウンが存在しない、または選択状態が最新インデックスと乖離している場合は再生成
  ...
  function updateHeaderActiveFormTitle(title) {
    ...
    if (badge) {
      ...
      if (title && activeTab !== 'dashboard') {
        ...
        select.id = 'header-active-form-select';
        select.style.background = 'transparent';
        select.style.border = 'none';
        select.style.color = 'var(--color-primary, #3182ce)';
        select.style.fontWeight = '700';
        select.style.fontSize = '0.8rem';
        select.style.cursor = 'pointer';
        select.style.outline = 'none';
        select.style.padding = '2px 18px 2px 2px';
        select.style.borderRadius = '4px';
        select.style.fontFamily = 'inherit';
        
        // 幅を短縮し、はみ出た部分は省略表示 (...) を適用
        ...
        const optgroupForms = document.createElement('optgroup');
        optgroupForms.label = '作成中のフォーム';
        optgroupForms.style.background = 'var(--color-bg-card, #151d30)';
        optgroupForms.style.color = 'var(--color-text, #fff)';
        
        allForms.forEach((f, idx) => {
          const opt = document.createElement('option');
          opt.value = `form_${idx}`;
          opt.textContent = f.title || `無題のフォーム ${idx + 1}`;
          opt.style.background = 'var(--color-bg-card, #151d30)';
          opt.style.color = 'var(--color-text, #fff)';
          if (!isTemplateMode && idx === activeIndex) {
            opt.selected = true;
          }
          optgroupForms.appendChild(opt);
        });
        select.appendChild(optgroupForms);

        if (templates.length > 0) {
          const optgroupTemplates = document.createElement('optgroup');
          optgroupTemplates.label = 'テンプレートマスタ';
          optgroupTemplates.style.background = 'var(--color-bg-card, #151d30)';
          optgroupTemplates.style.color = 'var(--color-text, #fff)';
          
          templates.forEach((t, idx) => {
            const opt = document.createElement('option');
            opt.value = `template_${idx}`;
            opt.textContent = t.title || `無題のテンプレート ${idx + 1}`;
            opt.style.background = 'var(--color-bg-card, #151d30)';
            opt.style.color = 'var(--color-text, #fff)';
            if (isTemplateMode && idx === activeIndex) {
              opt.selected = true;
            }
            optgroupTemplates.appendChild(opt);
          });
          select.appendChild(optgroupTemplates);
        }
        
        // 現在アクティブなタイトルの取得とツールチップ（ホバー）設定
        let currentTitle = '';
        ...
        select.title = currentTitle;
        badge.title = currentTitle;

        select.addEventListener('change', (e) => {
          const val = e.target.value;
          if (val.startsWith('form_')) {
            const idx = val.replace('form_', '');
            localStorage.setItem('form_customize_active_index', idx);
            localStorage.setItem('form_customize_is_template_mode', 'false');
          } else if (val.startsWith('template_')) {
            const idx = val.replace('template_', '');
            localStorage.setItem('form_customize_active_index', idx);
            localStorage.setItem('form_customize_is_template_mode', 'true');
          }
          // ホームに戻らず直接「フォーム編集」画面に遷移させる
  ...
  const DEFAULT_TEMPLATES = [
    {
      title: 'お客様フィードバック (テンプレート)',
      ownerId: 'user_all_editor',
      isLocked: false,
      sections: [
        {
          id: 'section_1',
          title: 'お客様情報',
          description: '基本的なお客様情報をご入力ください。',
          questions: [
            { id: 'q_name', type: 'text', title: 'お名前', required: true },
            { id: 'q_email', type: 'text', title: 'メールアドレス', required: true },
            { id: 'q_satisfaction', type: 'choice', title: '満足度', required: false, options: [
              { label: '満足', nextSectionId: 'next' },
              { label: '普通', nextSectionId: 'next' },
              { label: '不満', nextSectionId: 'next' }
            ]}
          ]
        }
      ]
    ...
    {
      title: 'アポイント記録 (テンプレート)',
      ownerId: 'user_own_editor',
      isLocked: false,
      sections: [
        {
          id: 'section_1',
          title: 'アポイント詳細',
          description: 'アポイントの概要を記録します。',
          questions: [
            { id: 'q_company', type: 'text', title: '企業名', required: true },
            { id: 'q_pic', type: 'text', title: 'ご担当者様名', required: false },
            { id: 'q_memo', type: 'paragraph', title: '打合せメモ', required: false }
          ]
        }
      ]
    }
  ...
  function initTemplates() {
    if (!localStorage.getItem('form_customize_templates')) {
      localStorage.setItem('form_customize_templates', JSON.stringify(DEFAULT_TEMPLATES));
    }
  }
  ...
  function getTemplates() {
    initTemplates();
    try {
      return JSON.parse(localStorage.getItem('form_customize_templates') || '[]');
    } catch(e) {
  ...
  function createFormFromTemplate(template) {
    ...
    const newForm = JSON.parse(JSON.stringify(template));
    newForm.title = newForm.title.replace(/\s*\(テンプレート\)/g, '').replace(/\s*\(sample\)/gi, '') + ' ' + new Date().toLocaleDateString();
    newForm.ownerId = 'user_own_editor';
    newForm.isLocked = false;

    allForms.push(newForm);
    localStorage.setItem('form_customize_all_forms', JSON.stringify(allForms));
    
    localStorage.setItem('form_customize_active_index', (allForms.length - 1).toString());
    localStorage.setItem('form_customize_is_template_mode', 'false');
    localStorage.setItem('form_customize_active_tab', 'editor');
    
    showCustomToast('テンプレートからフォームを作成しました。', 'success');
    setTimeout(() => {
      location.reload();
    }, 800);
  }

  function editTemplateDirectly(index) {
    localStorage.setItem('form_customize_active_index', index.toString());
    localStorage.setItem('form_customize_is_template_mode', 'true');
    localStorage.setItem('form_customize_active_tab', 'editor');
    
    showCustomToast('テンプレートの編集モードを起動します...', 'info');
    setTimeout(() => {
      location.reload();
    }, 800);
  }
  ...
  function registerFormAsTemplate(formIndex) {
    ...
    const newTemplate = JSON.parse(JSON.stringify(form));
    newTemplate.title = newTemplate.title.replace(/\s*\(テンプレート\)/g, '') + ' (テンプレート)';
    
    templates.push(newTemplate);
    saveTemplates(templates);

    showCustomToast(`フォーム「${form.title}」をテンプレートに登録しました！`, 'success');
  }
  ...
  function showCustomToast(msg, type = 'info') {
    const toast = document.createElement('div');
    ...
    toast.style.animation = 'fadeInBadge 0.2s ease-out';
  ...
  function renderTemplatesListOnDashboard() {
    ...
    const existingSection = document.getElementById('dashboard-templates-section');
    ...
    tplSection.id = 'dashboard-templates-section';
    tplSection.style.marginTop = '2.5rem';
    tplSection.style.borderTop = '1px dashed var(--color-border)';
    tplSection.style.paddingTop = '1.5rem';
    tplSection.style.paddingBottom = '2rem';

    tplSection.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
        <h3 style="margin: 0; font-size: 1.15rem; font-weight: 700; color: var(--color-text);">📂 登録済みテンプレートマスタ一覧</h3>
        <span style="font-size: 0.72rem; color: var(--color-text-muted);">マスタ自体を直接編集、または削除できます</span>
      </div>
      <div class="gf-list-container" id="dashboard-templates-list-container">
      </div>
    `;

    const container = tplSection.querySelector('#dashboard-templates-list-container');
    
    templates.forEach((tpl, idx) => {
      const row = document.createElement('div');
      row.className = 'gf-list-row';
      ...
      row.innerHTML = `
        <div class="gf-list-title-area" style="display: flex; align-items: center; gap: 10px; flex: 1;">
          <div class="purple-form-icon" style="background: #edf2f7; color: #4a5568; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 6px; font-weight: bold;">📁</div>
          <span style="font-weight: 600; color: var(--color-text);">${tpl.title}</span>
        </div>
        <div style="display: flex; gap: 0.5rem; align-items: center;">
          <button class="btn btn-sm btn-primary btn-use-template-row" data-index="${idx}" style="font-size: 0.72rem; padding: 4px 10px;">＋ フォーム作成</button>
          <button class="btn btn-sm btn-secondary btn-edit-template-row" data-index="${idx}" style="font-size: 0.72rem; padding: 4px 10px; color: var(--color-primary) !important;">✏️ マスタ編集</button>
          <button class="btn btn-sm btn-secondary btn-delete-template-row" data-index="${idx}" style="font-size: 0.72rem; padding: 4px 10px; color: #ef4444 !important; border-color: rgba(239,68,68,0.15) !important;">🗑️ 削除</button>
        </div>
      `;
    ...
    dashboardList.appendChild(tplSection);

    container.querySelectorAll('.btn-use-template-row').forEach(btn => {
    ...
    container.querySelectorAll('.btn-edit-template-row').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const idx = parseInt(btn.dataset.index);
        editTemplateDirectly(idx);
    ...
    container.querySelectorAll('.btn-delete-template-row').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const idx = parseInt(btn.dataset.index);
        deleteTemplate(idx);
  ...
  function startDashboardHookLoop() {
    setInterval(() => {
      ...
      if (sampleBtn && !sampleBtn.dataset.templated) {
        ...
        newSampleBtn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          openTemplateGallery();
        });
  ...
  function applyPreviewTheme() {
    ...
    if (isPro) {
      ...
      const titleSize = getActualFontSize(g.appearance.fontSizes.title, 'title', false);
      const sectionSize = getActualFontSize(g.appearance.fontSizes.section, 'section', false);
      const labelSize = getActualFontSize(g.appearance.fontSizes.label, 'label', false);
  ...
  function renderLivePreview() {
    ...
    if (isPro && g.appearance) {
      ...
      const titleSize = getActualFontSize(g.appearance.fontSizes.title, 'title', true);

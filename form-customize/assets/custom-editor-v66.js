...
(function() {
  ...
  function updateHeaderActiveFormTitle(title) {
    try {
      ...
      const shouldShowBadge = title && (activeTabFromStorage !== 'dashboard' || isEditorTabActive);
      ...
      if (badge) {
        if (shouldShowBadge) {
          // すでに正しい選択肢でドロップダウンが表示されている場合は、無駄な再生成をスキップしてフォーカスを維持
          const existingSelect = document.getElementById('header-active-form-select');
          const activeIndex = parseInt(localStorage.getItem('form_customize_active_index') || '0', 10);
          const isTemplateMode = localStorage.getItem('form_customize_is_template_mode') === 'true';
          const expectedValue = isTemplateMode ? `template_${activeIndex}` : `form_${activeIndex}`;

          if (existingSelect && existingSelect.value === expectedValue && badge.style.display === 'inline-flex') {
            console.log(`[Extension] Dropdown is already up-to-date. Skipping rebuild.`);
            return;
          }
          ...
          console.log(`[Extension] Building dropdown options:`, {
          ...
          console.log(`[Extension] Dropdown mounted successfully.`);
        } else {
          badge.style.display = 'none';
        }
      }
  ...
  function setupFlowmapOverrideF() {
    ...
    const customF = function(masterState) {
      ...
      if (masterState) {
        ...
        // isFormSwitchedの有無に関わらず、バッジが隠れている、あるいはプルダウン未生成の場合は常に更新を走らせて出現を担保する
        const badge = document.getElementById('active-form-title-badge');
        const badgeHidden = !badge || badge.style.display === 'none' || !document.getElementById('header-active-form-select');

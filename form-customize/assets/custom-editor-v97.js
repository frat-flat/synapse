...
(function() {
  ...
  function renderRichTextWithLinks(text) {
    ...
    escaped = escaped.replace(/&lt;u&gt;(.*?)&lt;\/u&gt;/gi, '<u style="text-decoration: underline;">$1</u>');
  ...
  // ==========================================
  function applyTextFormatting(input, action) {
    if (!input) return;
    input.focus();

    const start = input.selectionStart !== undefined ? input.selectionStart : (input.value ? input.value.length : 0);
    ...
    if (action === 'clear') {
      if (selected) {
        let cleared = selected
          ...
          .replace(/<\/?u>/gi, '')
          .replace(/\[(.*?)\]\([^)]+\)/g, '$1');
        input.value = val.substring(0, start) + cleared + val.substring(end);
        input.setSelectionRange(start, start + cleared.length);
      } else {
        let cleared = val
          .replace(/\*\*(.*?)\*\*/g, '$1')
          .replace(/\*(.*?)\*/g, '$1')
    ...
    if (action === 'bold') {
      toggleFormatWrap(input, '**', '**', '太字テキスト');
    } else if (action === 'italic') {
      toggleFormatWrap(input, '*', '*', '斜体テキスト');
    } else if (action === 'underline') {
      toggleFormatWrap(input, '<u>', '</u>', '下線テキスト');
    } else if (action === 'link') {
      if (typeof openLinkModal === 'function') {
        openLinkModal(input);
      } else if (window.openLinkModal) {
        window.openLinkModal(input);
      }
    }
  }

  function toggleFormatWrap(input, prefix, suffix, placeholder) {
    ...
    if (selected) {
      if (val.substring(start - pLen, start) === prefix && val.substring(end, end + sLen) === suffix) {
        input.value = val.substring(0, start - pLen) + selected + val.substring(end + sLen);
        input.setSelectionRange(start - pLen, end - pLen);
        ...
        input.value = val.substring(0, start) + inner + val.substring(end);
      ...
      input.value = val.substring(0, start) + prefix + placeholder + suffix + val.substring(end);
      input.setSelectionRange(start + pLen, start + pLen + placeholder.length);
    }
    input.dispatchEvent(new Event('input', { bubbles: true }));
    input.dispatchEvent(new Event('change', { bubbles: true }));
  }

  function createFormattingToolbar(targetInput) {
    ...
    bar.addEventListener('mousedown', (e) => {
      ...
      if (btn) {
        ...
        applyTextFormatting(targetInput, action);
      }
    });
  ...
  function injectRichTextToolbars() {
    const descInputs = document.querySelectorAll('.q-desc-input, .q-scroll-input, #editor-section-desc, #editor-form-desc, #pro-form-desc-input, #editor-pro-disclaimer');
    descInputs.forEach(input => {
      if (!input || !input.parentNode) return;
      if (input.nextElementSibling && input.nextElementSibling.classList.contains('rich-text-formatting-toolbar')) {
        return;
      }
      const bar = createFormattingToolbar(input);
      input.parentNode.insertBefore(bar, input.nextSibling);

      if (!input.dataset.fmtShortcutBound) {
        input.dataset.fmtShortcutBound = 'true';
        input.addEventListener('keydown', (e) => {
          if (e.ctrlKey || e.metaKey) {
            const key = e.key.toLowerCase();
            if (key === 'b') {
              e.preventDefault();
              applyTextFormatting(input, 'bold');
            } else if (key === 'i') {
              e.preventDefault();
              applyTextFormatting(input, 'italic');
            } else if (key === 'u') {
              e.preventDefault();
              applyTextFormatting(input, 'underline');
            }
          }
        });
      }
...
// Bank listeners moved to dedicated setup functions below
    ...
    const selectChanger = (e) => {
      ...
      if (val === 'pro_corp_address') {
        ...
        activeSec.questions.push(
          ...
          { id: `q_zip_${baseTime + 1}`, type: "text", title: "郵便番号", description: "法人選択または7桁入力で住所を自動補完します", required: true },
          { id: `q_pref_${baseTime + 2}`, type: "select", title: "都道府県", description: "お住まいの都道府県を選択してください", required: true, options: JAPAN_PREFECTURES.map(p => ({ label: p })) },
          { id: `q_city_${baseTime + 3}`, type: "text", title: "市区町村", description: "", required: true },
          { id: `q_street_${baseTime + 4}`, type: "text", title: "町名・番地", description: "", required: true },
          { id: `q_building_${baseTime + 5}`, type: "text", title: "建物名・部屋番号", description: "マンション名・ビル名・部屋番号等がある場合はご入力ください", required: false }
        );
        ...
        activeSec.questions.push(
          ...
          { id: `q_pref_${baseTime + 1}`, type: "select", title: "都道府県", description: "お住まいの都道府県を選択してください", required: true, options: JAPAN_PREFECTURES.map(p => ({ label: p })) },
          { id: `q_city_${baseTime + 2}`, type: "text", title: "市区町村", description: "", required: true },
          { id: `q_street_${baseTime + 3}`, type: "text", title: "町名・番地", description: "", required: true },
          { id: `q_building_${baseTime + 4}`, type: "text", title: "建物名・部屋番号", description: "マンション名・ビル名・部屋番号等がある場合はご入力ください", required: false }
        );
  ...
  function injectSameAsAboveEditorUI(qCard, qDef, sec) {
    ...
    const availableSources = getAvailableSourceQuestionsFor(sec.id, qDef.id);
    let sameContainer = qCard.querySelector('.same-as-above-editor-container');

    // コピー可能な前述の質問が1つもない（フォーム先頭の質問）場合
    ...
    if (!toggle) {
        ...
        <div class="same-as-above-details" style="display: none; flex-direction: column; gap: 8px; margin-top: 4px; padding-left: 22px;">
          <div style="display: flex; gap: 8px; align-items: flex-start; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 180px;">
              <label style="font-size: 0.72rem; color: #64748b; font-weight: 600; margin-bottom: 2px; display: block;">コピー元の質問（前述の項目）:</label>
          ...
          <div style="font-size: 0.72rem; color: #64748b; line-height: 1.4;">
            💡 回答者がチェックを入れると、前述の回答値が自動で入力欄に流し込まれます（目視確認可能）。
          </div>
        </div>
      `;
      ...
      toggle.addEventListener('change', () => {
        ...
        if (toggle.checked) {
          if (!qDef.sameAsAbove.sourceQuestionId || !availableSources.some(s => s.id === qDef.sameAsAbove.sourceQuestionId)) {
            qDef.sameAsAbove.sourceQuestionId = sourceSelect.value || (availableSources[0] ? availableSources[0].id : '');
          }
          sourceSelect.value = qDef.sameAsAbove.sourceQuestionId;

          if (!qDef.sameAsAbove.label || qDef.sameAsAbove.label.trim() === '') {
            const chosen = availableSources.find(s => s.id === qDef.sameAsAbove.sourceQuestionId);
      ...
      sourceSelect.addEventListener('change', () => {
        ...
        const oldSrc = availableSources.find(s => s.id === oldSrcId);

        qDef.sameAsAbove.sourceQuestionId = sourceSelect.value;
        const newSrc = availableSources.find(s => s.id === sourceSelect.value);
    ...
    // ドロップダウン選択肢の更新
    let optsHtml = '';
    const prevSecSources = availableSources.filter(s => !s.isSameSection);
    const sameSecSources = availableSources.filter(s => s.isSameSection);
    ...
    if (qDef.sameAsAbove && qDef.sameAsAbove.sourceQuestionId && availableSources.some(s => s.id === qDef.sameAsAbove.sourceQuestionId)) {
      sourceSelect.value = qDef.sameAsAbove.sourceQuestionId;
    } else if (availableSources[0]) {
      sourceSelect.value = availableSources[0].id;
      if (isEnabled && (!qDef.sameAsAbove.sourceQuestionId || !availableSources.some(s => s.id === qDef.sameAsAbove.sourceQuestionId))) {
        qDef.sameAsAbove.sourceQuestionId = availableSources[0].id;
      }
    }
  ...
  function setupEditorRenderHooks() {
    ...
    // アクティブセクション編集画面が表示されているときの一時保存UI・途中送信UIおよび書式ツールバーの自律維持
    setInterval(() => {
      injectRichTextToolbars();

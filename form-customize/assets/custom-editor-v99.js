...
// Bank listeners moved to dedicated setup functions below
  ...
  function injectSidebarDnDAndQuestionTree() {
    ...
    secItems.forEach((secItem) => {
      ...
      let needsRebuild = !existingUl || existingUl.children.length !== qList.length;
      if (!needsRebuild) {
        for (let i = 0; i < qList.length; i++) {
          if (existingUl.children[i].dataset.questionId !== qList[i].id) {
            needsRebuild = true;
            break;
          }
        }
      ...
      if (needsRebuild) {
        ...
        qList.forEach((q, qIdx) => {
          ...
          li.addEventListener('dragover', (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (!window._currentDnD || window._currentDnD.type !== 'question') return;
            if (window._currentDnD.questionId === q.id) return;
            e.dataTransfer.dropEffect = 'move';
          ...
          li.addEventListener('drop', (e) => {
            ...
            if (srcSecId === tgtSec.id) {
              const [movedQ] = tgtSec.questions.splice(fromIdx, 1);
    ...
    qCards.forEach((card, idx) => {
      ...
      if (!card._hasCardDnD) {
        ...
        card.addEventListener('dragover', (e) => {
          e.preventDefault();
          if (!window._currentDnD || window._currentDnD.type !== 'question') return;
          if (window._currentDnD.questionId === qId) return;
          e.dataTransfer.dropEffect = 'move';

...
// Bank listeners moved to dedicated setup functions below
  ...
  function patchPresetSelectMenu() {
    ...
    const selectChanger = (e) => {
      ...
      if (val === 'pro_corp_address') {
        ...
        activeSec.questions.push(
          { id: `q_zip_${baseTime}`, type: "text", title: "郵便番号", description: "7桁半角数字を入力すると住所を自動補完します", required: true, groupId: addrGrpId, groupTitle: addrGrpTitle },
          { id: `q_pref_${baseTime + 1}`, type: "select", title: "都道府県", description: "お住まいの都道府県を選択してください", required: true, options: JAPAN_PREFECTURES.map(p => ({ label: p })), groupId: addrGrpId, groupTitle: addrGrpTitle },
          { id: `q_city_${baseTime + 2}`, type: "text", title: "市区町村", description: "", required: true, groupId: addrGrpId, groupTitle: addrGrpTitle },
          { id: `q_street_${baseTime + 3}`, type: "text", title: "町名・番地", description: "", required: true, groupId: addrGrpId, groupTitle: addrGrpTitle },
          { id: `q_building_${baseTime + 4}`, type: "text", title: "建物名・部屋番号", description: "マンション名・ビル名・部屋番号等がある場合はご入力ください", required: false, groupId: addrGrpId, groupTitle: addrGrpTitle }
        );
  ...
  function injectQuestionGroupSystem(sec) {
    ...
    // Collect all unique groups in curSec for the group dropdown
    ...
    activeSec.questions.forEach(q => {
      ...
      if (q.groupId) {
        if (!currentPreviewGrpEl || currentPreviewGrpId !== q.groupId) {
          ...
          grpHeader.innerHTML = `<span>📁</span> <span>${escapeHtml(q.groupTitle || 'グループ')}</span>`;
